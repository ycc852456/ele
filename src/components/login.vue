<template>
    <div id="login">
        <div id="login_content">
            <div id="tit_logo">
                <svg viewBox="0 0 142 58" fill="#1B9DFF" fill-rule="evenodd">
            ...
            <div id="content_form">
                <form action="">
                    <div class="input" > 
                        <input type="text" maxlength="11" @input="write_phone"  name="phone" id="" placeholder="手机号" @focus="change_border">
                        <button id="get_yzm" disabled @click.prevent="get">获取验证码</button>
                    </div>
                    <div class="input" >
                        <input type="text"  name="yzm" placeholder="验证码" @focus="change_border">
                    </div>
                    <div id="xieyi">
                    ...
                    <div id="btn">
                        <button type="submit" @click.prevent="log">登录</button>
                    </div>
                    <div id="about">
                        <p>关于我们</p>
                    </div>
...
export default {
    ...
    methods:{
       change_border(e){
           $(".input").removeClass("input_active input_error")
        //    console.log(e.target)
           $(e.target.parentElement).addClass("input_active")
       },
        // 输入手机号
        write_phone(){
            ...
            if(result){
                $("#get_yzm").css({"color":"#2395ff"}).prop("disabled","")
                this.send=true
            }else{
                $("#get_yzm").css({"color":"#ccc"}).prop("disabled","disabled")
            }
        },
       get(){
           if(this.send){
                var code = (Math.random() * 1000000).toFixed(0)
                console.log(code)
                this.yzm=code
                var daojishi=10
                // 发送请求
                // this.$http.get("http://sms.tehir.cn/code/sms/api/v1/send?srcSeqId=123&account="+this.phone
                //                 +"&password=" + "111111"
                //                 +"&mobile=" + this.phone
                //                 + "&code=" + this.yzm
                //                 + "&time=15").then(function(resdata){
                //                     console.log(resdata)
                //                 })
                ...
                var timer= setInterval(function(){
                    ...
                    $("#get_yzm").css({"cursor":"not-allowed"}).text("已发送(" + daojishi + "s)")
                        if (daojishi == 0) {
                            clearInterval(timer)
                            $("#get_yzm").text("重新获取验证码").css({"cursor":"pointer","color":"#2395ff","fontSize":".75rem"}).prop("disabled", "")
                        }
                },1000)
            }else{
        ...
        log(){
            var input_code=$("input").eq(1).val()
            if (input_code==this.yzm) {
                // console.log("验证成功!")
                this.$vux.toast.show({
                    text: '验证成功'
                })
                this.$router.push({path:"/my",query:{phone:this.phone}})
                ...
                $(".input").eq(1).removeClass("input_active").addClass("input_error")
            }
        }
    },
    beforeMount() {
...
<style scoped>
    ...
    #content_form div.input button#get_yzm{
        border: none;
        width: 30%;
        background: #fff;
        margin-right: .5rem /* 8/16 */;
    ...
    .input_active{
        border: 1px solid #0089dc !important;
    }
    .input_error{
        border: 1px solid rgb(224, 102, 102) !important;
    }
